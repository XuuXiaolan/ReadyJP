...
public class ReadyJP : JohnPaularatusEnemyEnemyAI
{
    ...
    public override void Update()
    {
        ...
        if (Vector3.Distance(targetLungProp.transform.position, transform.position) <= 2.5f && grabLungPropRoutine == null)
        {
            ...
            {
                targetLungProp.playerHeldBy.DropAllHeldItemsServerRpc();
            }
    ...
    {
        creatureVoice.PlayOneShot(RandomDialogueSounds[JPRandom.Next(0, RandomDialogueSounds.Count)]);
    }
    ...
    public override void HitEnemy(int force = 1, PlayerControllerB? playerWhoHit = null, bool playHitSFX = false, int hitID = -1)
    {
        ...
        if (isEnemyDead) return;
        creatureVoice.PlayOneShot(DamageSounds[JPRandom.Next(0, DamageSounds.Count)]);
    ...
    public override void KillEnemy(bool destroy = false)
    {
        ...
        if (IsServer)
        {
            ...
            int randomDeathNumber = JPRandom.Next(1,3);
            ...
            if (targetLungProp == null) 
            {
                NetworkObjectReference lungPropRef = JohnPaularatusUtils.Instance.SpawnScrap(StartOfRound.Instance.allItemsList.itemsList.Where(x => x.itemName == "Apparatus").First(), RoundManager.Instance.GetRandomNavMeshPositionInBoxPredictable(this.transform.position, 4, default, JPRandom), false, true, 100);
            }
        }
        if (holdingLungProp)
        {
    ...
    {
        creatureSFX.PlayOneShot(FootstepSounds[JPRandom.Next(0, FootstepSounds.Length)]);
    }
}